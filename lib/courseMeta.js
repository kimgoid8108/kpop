...
export function findLesson(meta, partId, lessonId, instructorId) {
  ...
  const lesson =
    level.lessons.find((l) => l && (l.lessonId === lessonId || normalizeLessonId(l.lessonId) === normalized)) ?? null;
  if (!lesson) return null;
  if (instructorId && !sameInstructor(lesson.instructorId, instructorId)) return null;
  return lesson;
}
